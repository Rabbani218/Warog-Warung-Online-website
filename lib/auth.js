...
export const authOptions = {
  ...
  callbacks: {
    async jwt({ token, user, trigger, session }) {
      ...
      if (user) {
        token.id = user.id;
        token.role = user.role; // KUNCI: Pastikan role masuk ke token
        token.storeId = user.storeId;
        token.avatar = user.avatar;
      }

      // ▸ FORCE REFRESH: Jika role hilang atau CLIENT, coba fetch dari DB (Hardening)
      if (token?.id && (!token.role || token.role === "CLIENT")) {
        try {
          ...
          if (dbUser) {
            ...
            if (dbUser.storeId) token.storeId = dbUser.storeId;
          }
        } catch (err) {
          console.error("[Auth JWT] Error fetching role hardening:", err);
        }
      }
    ...
    async session({ session, token }) {
      if (token && session.user) {
        ...
        session.user.role = token.role; // KUNCI: Pindahkan role dari token ke session
