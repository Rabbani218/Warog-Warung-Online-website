...
export const authOptions = {
  ...
  callbacks: {
    async jwt({ token, user, trigger, session }) {
      ...
      // ▸ HARDEING: If token exists but role is missing, fetch it from DB
      // This prevents the "jump back to login" loop if the session is partially lost.
      if (token?.id && !token.role) {
        try {
          ...
          console.error("[Auth] Session hardening failed:", e);
        }
      }
