...
export const authOptions = {
  ...
  session: {
    strategy: "jwt",
    // 30 hari — session tidak expire terlalu cepat menyebabkan login loop
    maxAge: 30 * 24 * 60 * 60,
    // Update session setiap 24 jam
    updateAge: 24 * 60 * 60,
  },
  providers: authProviders,
  // PENTING: Jangan set pages.signIn ke '/admin' — itu untuk admin only
  // Kalau di-set, semua user CLIENT yang session-nya expire akan diredirect ke /admin
  // bukan ke halaman login yang benar, menyebabkan login loop
  pages: {
    // signIn hanya untuk admin panel, client menggunakan modal
    signIn: "/admin",
    error: "/admin",
  },
  callbacks: {
    async jwt({ token, user, trigger, session }) {
      ...
      if (user) {
        token.id = user.id;
        token.role = user.role; // KUNCI UTAMA: Injeksi role ke token
    ...
    async session({ session, token }) {
      if (token && session.user) {
        ...
        session.user.role = token.role; // KUNCI UTAMA: Pindahkan role dari token ke session
