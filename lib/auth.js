  ...
  callbacks: {
    async jwt({ token, user, account, trigger, session }) {
      ...
      if (!token.role && token.email) {
        ...
        token.bio = existingUser?.bio || null;
        token.address = existingUser?.address || null;
        token.hobbies = existingUser?.hobbies || null;
        token.favoriteFood = existingUser?.favoriteFood || null;
      }
      ...
      // Inject storeId securely
      if (!token.storeId && token.sub) {
        try {
