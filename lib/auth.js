...
export const authOptions = {
  ...
  callbacks: {
    async jwt({ token, user, trigger, session }) {
        ...
        if (!token.role || token.role === "CLIENT") {
          try {
            ...
            if (dbUser) {
              ...
              console.log(`[Auth JWT] Initial sign-in, role fetched from DB: ${token.role}`);
            }
          } catch (err) {
      ...
      // ▸ HARDEING: For subsequent requests, if data is missing, fetch from DB
      if (token?.id && (!token.role || (token.role === "ADMIN" && !token.storeId))) {
        try {
          ...
          console.error("[Auth JWT] Session hardening failed:", e);
        }
      }
